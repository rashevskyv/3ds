...
* Текстовый файл с читами для выбранной игры выбранного региона
	+ Наиболее полная база читов находится [здесь](http://www.fort42.com/gateshark/) ил [здесь](https://www.max-cheats.com). Но подойдут любые читы, совместимые с Gateway.
...
1. Закройте меню Rosalina, нажатием кнопки {% include inc/btn.txt btn="B" %} несколько раз
	...
	* Если какой-то чит-код подчеркнут красным - он не будет работать, поскольку адресует в неправильный регион памяти приставки
