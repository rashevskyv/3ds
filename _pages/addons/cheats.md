...
* Текстовый файл с читами для выбранной игры выбранного региона
	+ Наиболее полная база читов находится [здесь](http://www.fort42.com/gateshark/). Но подойдут любые читы, совместимые с Gateway.
...
1. Скопируйте `boot.firm` из `.zip-архива` Luma3DS в корень SD-карты, **соглашаясь на перезапись файлов**
...
## Часть III - CTRNAND Luma3DS

{% include /inc/ctrnand_luma.txt %}

## Часть IV - Использование читов
...
1. Закройте меню Rosalina, нажатием кнопки {% include inc/btn.txt btn="B" %} несколько раз
	* Если вместо списка читов вы видите надпись "No cheats found for title", вы скорее всего перепутали Title ID игры, либо не положили `cheats.txt` в соответствующую папку
	* Если вместо списка читов вы видите надпись "No suitable title found", вы запускаете читы из системного меню 


# Использование NTR CFW

Этот метод предполагает запуск еще одной кастомной прошивки поверх Luma3DS. Запуск NTR САЦ безопасен и не приведёт к брику. Читы, используемые в этом методе отличаются от тех, что использовались в предыдущем.
